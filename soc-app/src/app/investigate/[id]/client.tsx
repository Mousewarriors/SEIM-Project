...
import {
    ...
    CheckCircle2, XCircle, AlertTriangle, ArrowRight, Trophy, Target, X, PlayCircle, Scale
} from 'lucide-react';
...
export default function InvestigationClient({ scenario, initialEvents }: { scenario: any; initialEvents: EventLog[] }) {
    ...
    const { setAlertStatus, setPlaybook, setVerdictCorrectness, activePlaybook, setInvestigationContext } = useAlertStore();
    ...
    useEffect(() => {
        ...
        // Push context to Global AI
        setInvestigationContext({
            alertId: scenario.alert.id,
            alertTitle: scenario.alert.title,
            description: scenario.alert.description || '',
            severity: scenario.alert.severity,
            host: scenario.alert.host,
            user: scenario.alert.user || 'Unknown',
            iocs: scenario.iocs || { ips: [], domains: [] },
            eventCount: initialEvents.length,
            playbookSteps: scenario.playbook?.steps || [],
            logs: initialEvents.slice(0, 50).map(e => ({
                timestamp: e['@timestamp'],
                dataset: e.event.dataset,
                action: e.event.action,
                process: e.process?.name,
                destination: e.destination?.ip,
                source: e.source?.ip,
                message: e.event.original || e.message || ''
            }))
        });

    }, [scenario, scenarioId, setPlaybook, setInvestigationContext, initialEvents]);
...
function MitrePanel({ scenario }: { scenario: any }) {
    ...
    const runAnalysis = async () => {
        ...
        try {
            ...
            const payload = {
                alertDescription: `TITLE: ${scenario.alert.title}\n\nDESCRIPTION: ${scenario.alert.description}\n\nHOST: ${scenario.alert.host}\n\nSEVERITY: ${scenario.alert.severity}`
            };
            ...
            if (!data.results) throw new Error("No results returned from analyzer");
    ...
    return (
        <div style={{ padding: '40px', maxWidth: '800px', margin: '0 auto' }}>
            <div style={{ textAlign: 'center', marginBottom: '32px' }}>
                <div style={{ display: 'inline-flex', padding: '12px', background: 'rgba(239,68,68,0.1)', borderRadius: '12px', marginBottom: '16px' }}>
                    <Scale size={32} color="var(--red)" />
                </div>
                <h2 style={{ fontSize: '20px', fontWeight: 700, marginBottom: '8px', color: 'var(--text-primary)' }}>MITRE ATT&CK Analysis</h2>
                <p style={{ color: 'var(--text-secondary)', fontSize: '14px', maxWidth: '480px', margin: '0 auto', lineHeight: 1.5 }}>
                    Identify potential adversary techniques by analyzing this alert against the MITRE Knowledge Base.
                </p>
            </div>
            ...
            {!results && !loading && (
                <div style={{ textAlign: 'center' }}>
                    <button
                        onClick={runAnalysis}
                        className="btn btn-primary"
                        style={{ padding: '12px 32px', fontSize: '14px', height: 'auto', background: 'var(--red)', borderColor: 'var(--red)' }}
                    >
                        <PlayCircle size={18} style={{ marginRight: '8px' }} />
                        Run Threat Analysis
                    </button>
                    {error && (
                        <div style={{ marginTop: '24px', padding: '12px', background: 'rgba(239,68,68,0.1)', color: 'var(--red)', borderRadius: '6px', fontSize: '13px', display: 'inline-block' }}>
                            Error: {error}
                        </div>
                    )}
                </div>
            ...
            {loading && (
                <div style={{ textAlign: 'center', padding: '40px' }}>
                    ...
                    <div style={{ color: 'var(--text-muted)', fontSize: '13px' }}>Analyzing alert patterns...</div>
                </div>
            )}
            ...
            {results && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', animation: 'fadeIn 0.3s ease-out' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                        <div style={{ fontWeight: 700, fontSize: '13px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Top Suspected Techniques</div>
                        <button onClick={runAnalysis} className="btn btn-ghost" style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                            <RotateCcwIcon size={12} style={{ marginRight: '4px' }} /> Re-run
                        </button>
                    </div>

                    {results.map((tech: any) => (
                        <div key={tech.technique_id} className="card" style={{
                            padding: '16px',
                            borderLeft: `4px solid ${tech.relevance > 80 ? 'var(--red)' : tech.relevance > 50 ? 'var(--amber)' : 'var(--border)'}`,
                            background: 'var(--bg-secondary)'
                        }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    ...
                                    <span style={{ fontWeight: 600, color: 'var(--text-primary)', fontSize: '14px' }}>{tech.name}</span>
                                </div>
                                <span className="badge" style={{
                                    background: tech.relevance > 80 ? 'rgba(239,68,68,0.1)' : 'var(--bg-tertiary)',
                                    color: tech.relevance > 80 ? 'var(--red)' : 'var(--text-muted)',
                                    border: '1px solid transparent'
                                }}>
                                    {Math.round(tech.relevance)}% Match
                                </span>
                            </div>

                            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: '0 0 12px 0', lineHeight: 1.5 }}>
                                {tech.description_snippet}
                            </p>

                            <div style={{ display: 'flex', gap: '16px', fontSize: '11px', color: 'var(--text-muted)' }}>
                                <div>
                                    <strong style={{ color: 'var(--text-secondary)' }}>Tactics:</strong> {tech.tactics}
                                </div>
                                <div>
                                    <strong style={{ color: 'var(--text-secondary)' }}>Platforms:</strong> {tech.platforms}
                                </div>
                                {tech.url && (
                                    <a href={tech.url} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--cyan)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                        View on MITRE <Globe size={10} />
                                    </a>
                                )}
                            </div>
                        </div>
...
function RotateCcwIcon({ size, style }: any) {
    // Simple inline SVG if not imported, or reuse RotateCcw from lucide (need to import it first)
    // I forgot to import RotateCcw in the top block! I'll define it here or use existing imports.
    // Wait, Is RotateCcw imported?
    // Looking at line 8-10... No.
    // I can stick it in the imports or simpler: replace with ArrowRight? 
    // Actually I'll just add it to imports in the first chunk? 
    // No I already submitted the chunks. 
    // I'll assume users won't click re-run immediately or I'll fix the import in a second pass if it breaks.
    // BETTER: Use 'Trophy' or something I imported? No.
    // I will use a simple text or 'Target' icon which IS imported.
