...
import { Bot, Send, Sparkles, StickyNote, Trash2, X, Shield, ClipboardCheck, ChevronDown, ChevronRight, Check, Scale, Loader, Globe } from 'lucide-react';
...
export default function GlobalPanel() {
    const {
        ...
        isVerdictCorrect, currentInvestigationContext
    } = useAlertStore();
    ...
    const [mitreResults, setMitreResults] = useState<any[] | null>(null);
    ...
    const runMitreAnalysis = async () => {
        if (!notepadText.trim()) {
            alert("Please add some notes to analyze first.");
            return;
        }
        ...
        try {
            const res = await fetch('/api/mitre', {
                ...
                body: JSON.stringify({ alertDescription: notepadText })
            });
            const data = await res.json();
            if (data.error) throw new Error(data.error);
            setMitreResults(data.results || []);
        } catch (e) {
            console.error(e);
        } finally {
        ...
        try {
            ...
            const res = await fetch('/api/ai-chat', {
                ...
                body: JSON.stringify({
                    ...
                    context: currentInvestigationContext ? {
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <textarea
                            ...
                            disabled={aiLoading}
                        />
                        <button onClick={handleAiSend} className="btn btn-primary" style={{ padding: '6px 10px', height: 'auto' }} disabled={aiLoading}>
                            <Send size={14} />
                        </button>
                    </div>
                </div>
            ...
            {notepadOpen && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', gap: '4px' }}>
                            <PanelBtn icon={<Scale size={13} />} title="Analyze findings with MITRE" onClick={runMitreAnalysis} />
                        ...
                        <div onClick={e => e.stopPropagation()} style={{
                            ...
                            <div style={{ padding: '16px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    <Scale size={18} color="var(--red)" />
                                    <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>MITRE ATT&CK Analysis</span>
                                </div>
                                <button onClick={() => setShowMitreModal(false)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}>
                                    <X size={18} />
                                </button>
                            </div>
                            <div style={{ flex: 1, overflowY: 'auto', padding: '0', background: 'var(--bg-primary)' }}>
                                {mitreLoading ? (
                                    <div style={{ padding: '60px', textAlign: 'center' }}>
                                        <div style={{
                                            width: '24px', height: '24px', border: '2px solid var(--border)',
                                            borderTopColor: 'var(--cyan)', borderRadius: '50%', margin: '0 auto 16px',
                                            animation: 'spin 1s linear infinite'
                                        }}>
                                            <style>{`@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }`}</style>
                                        </div>
                                        <div style={{ color: 'var(--text-muted)', fontSize: '13px' }}>Analyzing your notes against MITRE KB...</div>
                                    </div>
                                ) : mitreResults ? (
                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        {mitreResults.length === 0 && (
                                            <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '13px' }}>
                                                No matching techniques found based on your notes.
                                            </div>
                                        )}
                                        {mitreResults.map((tech: any) => (
                                            <div key={tech.technique_id} style={{
                                                ...
                                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                        ...
                                                        <span style={{ fontWeight: 600, color: 'var(--text-primary)', fontSize: '13px' }}>{tech.name}</span>
                                                    </div>
                                                    <span className="badge" style={{
                                                        background: tech.relevance >= 100 ? 'rgba(0, 255, 157, 0.1)' :
                                                            tech.relevance >= 75 ? 'rgba(255, 176, 32, 0.1)' : 'var(--bg-tertiary)',
                                                        color: tech.relevance >= 100 ? 'var(--green)' :
                                                            tech.relevance >= 75 ? 'var(--amber)' : 'var(--text-muted)',
                                                        ...
                                                        {tech.relevance >= 100 ? '✅ CONFIRMED' :
                                                            tech.relevance >= 75 ? '⚠️ POTENTIAL' :
                                                ...
                                                <div style={{ display: 'flex', gap: '12px', fontSize: '10px', color: 'var(--text-muted)' }}>
                                                    <span><strong>Tactics:</strong> {tech.tactics}</span>
                                                    {tech.url && (
                                                        <a href={tech.url} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--cyan)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '3px' }}>
                                                            MITRE <Globe size={10} />
                                                        </a>
                                                    )}
                                                </div>
