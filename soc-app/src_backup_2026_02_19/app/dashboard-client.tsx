    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: '32px 40px', overflow: 'hidden' }}>
            ...
            <div className="card" style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{
                    display: 'grid',
                    gridTemplateColumns: '40px 100px 1fr 140px 150px 120px 40px',
                    padding: '12px 16px',
        ...
        <div style={{ borderBottom: '1px solid var(--border)' }}>
            ...
            >
                ...
                <div style={{ fontSize: '12px', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <Monitor size={12} /> {s.alert.host || 'Unknown'}
                </div>
                ...
                <div>
                    <span className="badge" style={{
                        background: status === 'investigating' ? 'rgba(6,182,212,0.1)' : status === 'closed' ? 'rgba(34,197,94,0.1)' : 'var(--bg-tertiary)',
                        color: status === 'investigating' ? 'var(--cyan)' : status === 'closed' ? 'var(--green)' : 'var(--text-muted)'
                    }}>
                        {status.toUpperCase()}
                    </span>
                </div>
            ...
            {isExpanded && (
                <div style={{ background: 'var(--bg-primary)', padding: '16px 20px 16px 68px', borderTop: '1px solid var(--border)' }}>
                    <div style={{ marginBottom: '16px', fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                        <div style={{ fontWeight: 700, color: 'var(--text-muted)', fontSize: '11px', textTransform: 'uppercase', marginBottom: '4px' }}>Description</div>
                        {s.alert.description}
                    </div>
